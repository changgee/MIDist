...
SimIMICE = function(scen,R,n,M,init,iter,batch=0)
{
  K = length(n)
  method = sprintf("IMICE%d",K)
...
IMICE = function(d,M,init,iter,n)
{
  p = d$p+1
  N = sum(n)
  K = length(n)
  ...
  for ( m in 1:M )
  {
    for ( it in 1:init )
    {
      for ( j in 3:5 )
      {
        ...
        fit.imp = LS(d,j,Xidx,n)

        for ( k in 1:K )
          if ( fit.imp$df[k] > 0 )
          {
            sig = sqrt(1/rgamma(1,(fit.imp$df[k]+1)/2,(fit.imp$SSE[k]+1)/2))
            alpha = fit.imp$beta[,k] + sig * backsolve(fit.imp$cgram[,,k],rnorm(p))
