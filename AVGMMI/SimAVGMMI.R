...
SimAVGMMI = function(scen,R,n,M,batch=0)
{
  ...
  for ( r in 1:R )
  {
    ...
    fit = AVGMMI(d,M,midx,MImethod,n)
    Theta[,r] = fit$theta
    comm[r] = fit$comm
  }
...
AVGMMI = function(d,M,midx,method,n)
{
  p = d$p+1
  q = length(midx)
  N = sum(n)
  ...
  {
    if ( method[j] == "norm" )
      fit.imp[[j]] = AVGMLS(d,midx[j],setdiff(1:(p+1),midx),n)
    else
      fit.imp[[j]] = AVGMLogit(d,midx[j],setdiff(1:(p+1),midx),n,c(0,d$alpha))
